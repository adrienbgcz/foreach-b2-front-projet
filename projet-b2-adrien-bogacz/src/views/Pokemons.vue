...
export default {
  ...
  computed: {
    filteredPokemons: function () {
      ...
      return this.pokemons.results?.filter((pokemon) =>
        pokemon.name.includes(this.inputValue)
      );
    },
    getLocalesArray() {
      let locales = [];
      this.$i18n.locale === ViewsConstants.SHORTCUT_FRENCH
        ? (locales = [
            ViewsConstants.FRENCH_IN_FRENCH,
            ViewsConstants.ENGLISH_IN_FRENCH,
          ])
        : (locales = [
            ViewsConstants.FRENCH_IN_ENGLISH,
            ViewsConstants.ENGLISH_IN_ENGLISH,
          ]);
      return locales;
    },
  },
  ...
  methods: {
    ...
    incrementCounterImage() {
      this.counterImage++;
      if (this.pokemons.results?.length === this.counterImage)
    ...
    convertLocaleToName(locale) {
      if (this.$i18n.locale === ViewsConstants.SHORTCUT_ENGLISH) {
        if (locale === ViewsConstants.SHORTCUT_FRENCH)
          return ViewsConstants.FRENCH_IN_ENGLISH;
        if (locale === ViewsConstants.SHORTCUT_ENGLISH)
          return ViewsConstants.ENGLISH_IN_ENGLISH;
      }
      if (this.$i18n.locale === ViewsConstants.SHORTCUT_FRENCH) {
        if (locale === ViewsConstants.SHORTCUT_FRENCH)
          return ViewsConstants.FRENCH_IN_FRENCH;
        if (locale === ViewsConstants.SHORTCUT_ENGLISH)
          return ViewsConstants.ENGLISH_IN_FRENCH;
      }
    },
    convertNameToLocale(name) {
      if (
        name.toLowerCase() === ViewsConstants.ENGLISH_IN_ENGLISH_LOWER_CASE ||
        name.toLowerCase() === ViewsConstants.ENGLISH_IN_FRENCH_LOWER_CASE
      )
        return ViewsConstants.SHORTCUT_ENGLISH;
      if (
        name.toLowerCase() === ViewsConstants.FRENCH_IN_ENGLISH_LOWER_CASE ||
        name.toLowerCase() === ViewsConstants.FRENCH_IN_FRENCH_LOWER_CASE
      )
        return ViewsConstants.SHORTCUT_FRENCH;
    },
  },
};
</script>
